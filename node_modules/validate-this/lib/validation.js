...
function validator() {
  ...
  function customValidator(fields) {
    return Object.keys(customRules).reduce(function (v, name) {
      ...
      satisfies: function satisfies() {
        for (var _len = arguments.length, rules = Array(_len), _key = 0; _key < _len; _key++) {
          rules[_key] = arguments[_key];
        }

        return validateFields(rules, fields);
      }
    });
  }
  ...
  var v = {
    validateChild: function validateChild(field, childValidations) {
      var childTranslator = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : translator;

      var childErrors = validator(values[field] || {}, childValidations, childTranslator);
    ...
    validateChildren: function validateChildren(field, childValidations) {
      ...
      var childErrors = (values[field] || []).map(function (v) {
        return validator(v, childValidations, childTranslator);
      });
      if (childErrors.some(function (e) {
